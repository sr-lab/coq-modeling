...
class DecoderLocalWrapper:
    ...
    def get_recs(
        ...
        return ModelResult(tactics, scores, lengths, [0])
    ...
    def from_checkpoint(cls, checkpoint_loc: Path, conf: dict[str, Any]) -> DecoderLocalWrapper:
        ...
        model, tokenizer = get_processed_model(str(checkpoint_loc.resolve()), conf["checkpoint_loc"], training_conf)
        model.to("cuda")
...
class UnslothDecoderLocalWrapper:
    ALIAS = "unsloth-decoder-local"
...
def wrapper_from_conf(conf: Any) -> ModelWrapper:
    ...
    match attempted_alias:
        ...
        case UnslothDecoderLocalWrapper.ALIAS:
            return UnslothDecoderLocalWrapper.from_conf(conf)
        case _:
